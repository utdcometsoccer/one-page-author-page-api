...
class Program
{
    // List of specific containers to seed as specified in the issue
    private static readonly string[] TargetContainers = new[]
    {
        "ArticleList",
        "AuthorDocList",
        "AuthorMainForm",
                                ...
                                .Build())
        {
            ...
            foreach (var file in jsonFiles)
            {
                ...
                foreach (var property in root.EnumerateObject())
                {
                    ...
                    // Only process containers specified in the issue
                    if (!TargetContainers.Contains(containerName))
                    {
                        Console.WriteLine($"Skipping container {containerName} - not in target list");
                        continue;
                    }
